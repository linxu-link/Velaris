 ...
 * 获取本地图片媒体列表。
 *
 * 支持排序和分页参数，供背景选择等场景复用。
 */
class GetMediaImagesUseCase @Inject constructor(private val mediaRepository: MediaRepository) {
    suspend operator fun invoke(
        sortOrder: MediaSortOrder = MediaSortOrder.DATE_DESC,
        limit: Int? = null,
        offset: Int? = null,
    ): List<MediaItem> = mediaRepository.getImages(sortOrder, limit, offset)
}
