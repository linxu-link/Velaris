 ...
 * 更新场景控制设置。
 *
 * 包括亮度、遮罩、计时器、时钟等播放期控制项。
 */
class UpdateSceneControlSettingsUseCase @Inject constructor(
    private val sceneResourceRepository: SceneResourceRepository,
) {
    suspend operator fun invoke(sceneId: String, settings: SceneControlSettings) =
        sceneResourceRepository.updateSceneControlSettings(sceneId, settings)
}
