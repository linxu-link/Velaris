 ...
 * 获取本地视频媒体列表。
 *
 * 支持排序和分页参数，供视频选择面板按需读取。
 */
class GetMediaVideosUseCase @Inject constructor(private val mediaRepository: MediaRepository) {
    suspend operator fun invoke(
        sortOrder: MediaSortOrder = MediaSortOrder.DATE_DESC,
        limit: Int? = null,
        offset: Int? = null,
    ): List<MediaItem> = mediaRepository.getVideos(sortOrder, limit, offset)
}
